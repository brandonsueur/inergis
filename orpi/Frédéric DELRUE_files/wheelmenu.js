...
!function($){
  ...
  $.fn.flyIn = function (el, button, width, height, angle, step, radius, settings) {
    ...
    this.each(function(index) {
      ...
      var x = Math.round(width/2 + radius * Math.cos(angle) - $(this).find("a").outerWidth()/1),
          y = Math.round(height/2 + radius * Math.sin(angle) - $(this).find("a").outerHeight()/1);
